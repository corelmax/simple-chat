...
const getPersistendMessageRequest = createAction(GET_PERSISTEND_MESSAGE, (paylaod) => paylaod);
const getPersistendMessageCancel = createAction(GET_PERSISTEND_MESSAGE_CANCELLED);
const getPersistendMessageSuccess = createAction(GET_PERSISTEND_MESSAGE_SUCCESS, (payload) => payload);
const getPersistendMessageFailure = createAction(GET_PERSISTEND_MESSAGE_FAILURE, (error) => error);
export const getPersistendMessage = (roomId) => __awaiter(this, void 0, void 0, function* () {
    getStore().dispatch(getPersistendMessageRequest(roomId));
    try {
        const result = yield ChatRoomComponent.getInstance().getPersistentMessage(roomId);
...
export const updateMessagesRead = createAction(UPDATE_MESSAGES_READ, (messages, roomId) => ({ messages, roomId }));
...
export const uploadFile = (progressEvent, file) => ({ type: CHATROOM_UPLOAD_FILE, payload: { data: progressEvent, file } });
